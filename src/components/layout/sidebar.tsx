  ...
  return (
    <>
      ...
      <div className={`hidden lg:flex flex-col justify-between bg-white px-4 py-4 md:px-7 md:py-6 fixed sm:top-[90px] top-[69px] z-50 sm:h-[calc(100vh-88px)] h-[calc(100vh-85px)] lg:px-7 ${className}`}>
        ...
        {isAuthenticated && (
          <div className="flex gap-3 cursor-pointer py-4" onClick={logout}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className={`fixed right-0 left-auto z-50 lg:hidden flex flex-col justify-between bg-white px-4 py-4 md:px-7 md:py-6 sm:top-[90px] top-[66px] sm:h-[calc(100vh-90px)] h-[calc(100vh-66px)] lg:px-7 shadow-[-5px_0_5px_-5px_rgba(0,0,0,0.1)] w-[250px] ${className}`}
          >
            ...
            {isAuthenticated ? (
              ...
              <div className="flex flex-col items-start gap-3 mb-4">
                <Link to="/login" className="flex justify-center py-2 px-4 border border-transparent rounded-xl shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 cursor-pointer">
            ...
            {/* Logout Button - Fixed positioning for mobile */}
            {isAuthenticated && (
              <div 
                className="flex gap-3 cursor-pointer py-4 px-4 mt-2 mb-1 w-full"
                onClick={logout}
              >
